...
class Buffer:
    ...
    def skip_if_startswith(self, data) -> bool:
...
def parse_decoding(decoding) -> Callable[[...]]:
    if decoding is None:
        return bytes
...
class RedisRespDecoder:
    # TODO (misc) maybe add decoding, and push_decoder ?
    def __init__(self, decoding=None, attributes=False, **kwargs):
        self._decoder = parse_decoding(decoding)
    ...
    def _extract_generator(self):
        ...
        while True:
                    ...
                    while True:
                        ...
                        while True:
                            if len(buffer) >= chunk_size + 2:
                                break
                            yield _need_more_data
                        chunks.append(buffer.read(chunk_size))
                        buffer.skip(2)
                    ...
                    if length == -1:
                        ...
                        while True:
                            if len(buffer) >= length + 2:
                                break
                            yield _need_more_data
                        msg = self._decoder(buffer.read(length))
                        buffer.skip(2)
            # Array
            elif buffer.skip_if_startswith(b"*"):
                while True:
            ...
            # End of streaming aggregate type
                ...
                raise ProtocolError("Unknown type: %s" % bytes(buffer.read(1)).decode())
